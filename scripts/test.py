...
import pandas as pd
from project_utils.my_utils import set_seed,load_model,nn,create_list,test_kmeans_hidisc
...
source_domains_mapping = {
    'Office_Home': {
        "Art": f"checkpoints/{args.dataset_name}/Art/Intermediate_Art_trained_model_{args.checkpoint}.pkl",
        "Clipart": f"checkpoints/{args.dataset_name}/Clipart/Intermediate_Clipart_trained_model_{args.checkpoint}.pkl",
        "Product": f"checkpoints/{args.dataset_name}/Product/Intermediate_Product_trained_model_{args.checkpoint}.pkl",
        "Real_world": f"checkpoints/{args.dataset_name}/Real_world/Intermediate_Real_world_trained_model_{args.checkpoint}.pkl"
    },
    'PACS': {
        "art_painting": f"checkpoints/{args.dataset_name}/art_painting/Intermediate_art_painting_trained_model_{args.checkpoint}.pkl",
        "photo": f"checkpoints/{args.dataset_name}/photo/Intermediate_photo_trained_model_{args.checkpoint}.pkl",
        "cartoon": f"checkpoints/{args.dataset_name}/cartoon/Intermediate_cartoon_trained_model_{args.checkpoint}.pkl",
        "sketch": f"checkpoints/{args.dataset_name}/sketch/Intermediate_sketch_trained_model_{args.checkpoint}.pkl"
    },
    'Domain_Net': {
        "clipart": f"checkpoints/{args.dataset_name}/clipart/Intermediate_clipart_trained_model_{args.checkpoint}.pkl",
        "sketch": f"checkpoints/{args.dataset_name}/sketch/Intermediate_sketch_trained_model_{args.checkpoint}.pkl",
        "painting": f"checkpoints/{args.dataset_name}/painting/Intermediate_painting_trained_model_{args.checkpoint}.pkl",
    }
}
...
selected_classes=create_list(source_domain=os.path.join(f"/janaki/backup/users/student/pg/pg23/vaibhav.rathore/datasets/{args.dataset_name}", all_domains[0]), num_classes=num_classes_mapping[args.dataset_name])
    ...
    for domain in all_domains:
        if domain!=train_domain:
            ...
            target_Dataloader = create_test_dataloaders(
                # For Source Domain Name replace the prefix with the actual path of your dataset . For my case it was on different path than the current scripts.
                target_domain=os.path.join(f"/janaki/backup/users/student/pg/pg23/vaibhav.rathore/datasets/{args.dataset_name}", domain),
            ...
            try:
                with torch.no_grad():
                    ...
                    all_acc_test, old_acc_test, new_acc_test = test_kmeans_hidisc(
                        device=device,
                        model=adapted_model,
...
save_path = f"HIDISC/Results/{args.dataset_name}/HIDISC_Results.xlsx"
